...
def plane_downsample(input_tensor, scale_factor):
    downsampled = F.avg_pool2d(input_tensor, kernel_size=scale_factor, stride=scale_factor)

    block_ones = (input_tensor == 1).all(dim=1, keepdim=True).float()  
    downsampled_block_ones = F.avg_pool2d(block_ones, kernel_size=scale_factor, stride=scale_factor)
    
    downsampled[downsampled_block_ones > 0] = 1
