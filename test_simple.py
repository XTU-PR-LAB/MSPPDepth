...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Simple testing funtion for MSPPDepth models.')
...
def depth_to_disp(depth, min_depth, max_depth):
    ...
    # Calculate scaled disparity from depth
    scaled_disp = 1 / depth
    
    # Convert scaled disparity to disparity in the range [0, 1]
...
def test_simple(args):
    ...
    with torch.no_grad():
        for idx, image_path in enumerate(paths):
            ...
            scaled_disp = 1/depth
            disp = depth_to_disp(depth, 0.1, 100)
            # disp = outputs[("disp", 0)]
