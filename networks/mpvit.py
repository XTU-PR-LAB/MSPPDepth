    ...
    def __init__(
        ...
        self.patch_embed_stages = nn.ModuleList(
            [
                ...
                for idx in range(self.num_stages)#4
            ]
        )
...
def mpvit_small(**kwargs):
    ...
    checkpoint = torch.load('/home/aoao/n417/dm/MonoViT_main/ckpt/mpvit_small.pth', map_location=lambda storage, loc: storage)['model']
    logger = get_root_logger()
    load_state_dict(model, checkpoint, strict=False, logger=logger)
    del checkpoint
    del logger
